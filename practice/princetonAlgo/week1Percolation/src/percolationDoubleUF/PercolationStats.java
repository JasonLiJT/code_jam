    ...
    public PercolationStats(int n, int trials) {
        ...
        for (int i = 0; i < trials; i++) {
            ...
            while (!percolation.percolates()) {
                int currentSite, row, col;
                do {
                    currentSite = StdRandom.uniform(n * n);
                    row = rowFromZeroBased(currentSite);
                    col = colFromZeroBased(currentSite);
                } while (percolation.isOpen(row, col));
                percolation.open(row, col);
            }
            trialResults[i] = 1.0 * percolation.numberOfOpenSites() / n / n;
        }
        if (n == 1) {
            this.mean = 0.5;
